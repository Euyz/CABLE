...
class CustomIMSDataset(Dataset):
    def __init__(self, pb, rb, tb, data, z0=0):
        self.pb = pb
        self.rb = rb
        self.tb = tb
        self.data = data
    ...
    def getDataByNiiIndexFromTif(self, tif, nii_roi):
        ...
        temp_flag = self.axisTransform(self.flag, nii_roi)
        return torch.tensor(temp.astype(np.float32)), temp_flag.copy()

    def axisTransform(self, tif, nii_roi):
        shape = tif.shape
        temp = tif[shape[0] - nii_roi[1][1] : shape[0] - nii_roi[1][0],
                   shape[1] - nii_roi[2][1] : shape[1] - nii_roi[2][0],
                   shape[2] - nii_roi[0][1] : shape[2] - nii_roi[0][0]]
    ...
    def __getitem__(self, i):
        if not hasattr(self, 'img_hdf5'):
            self.open_hdf5()
        return self.getDataByNiiIndexFromTif(self.data, self.pb[i])
